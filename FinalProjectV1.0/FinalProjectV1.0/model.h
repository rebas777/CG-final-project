...
private:
	...
	void loadModel(string path)
	{
		...
		// Process ASSIMP's root node recursively 递归调用，从根节点开始处理
	...
	vector<Texture2D> loadMaterialTextures(aiMaterial* mat, aiTextureType type, string typeName)
	{
		...
		for (GLuint i = 0; i < mat->GetTextureCount(type); i++)
		{
			...
			{   // If texture hasn't been loaded already, load it
				...
				ResourceManager::LoadTexture(filename.c_str(), GL_TRUE, filename);
				Texture2D texture = ResourceManager::GetTexture(filename);
